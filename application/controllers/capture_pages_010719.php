...
class capture_pages extends CI_Controller {
	...
	public function pay(){
        ...
        $stripe = array(
		  "secret_key"      => "sk_test_Zrr0NO45czflGaEYV0Sio7zX00rbxwYttA",
		  "publishable_key" => "pk_test_AxZasm8WBASNkFVMYn9h3Vyu00sOeSwJKB"
		);
		...
		if (isset($_POST['stripeToken'])){
	        try {	
				...
				/*$product = \Stripe\Product::create([
				  'name' => 'Monthly Capture Page Service',
				  'type' => 'service',
				]);
				print_r($product);
				die();*/
				...
				$customer = \Stripe\Customer::create(array(
					...
					"source" => $_POST['stripeToken'], // The token submitted from Checkout0
				));
				...
				$userdata = $this->capture_pages_model->get_userdata($usercode);
	            $referralid = $userdata['referralid_free'];
	          	$this->capture_pages_model->update($data,'membermaster','usercode',$usercode);
	            //
	            ...
	            $this->capture_pages_model->addItem($data,'tbl_payment_stripe');

	            echo "<script>alert('Payment completed Successfully!'); window.location.href='".base_url()."index.php/capture_pages/'</script>";
	            //redirect('upgrade_membership/view/');
	            
	        }

	        catch(\Stripe\Error\Card $e) {
	        	echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.1";
	        }
			catch (\Stripe\Error\RateLimit $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.2";
		    } 
		    catch (\Stripe\Error\InvalidRequest $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.3";
		    } 
		    catch (\Stripe\Error\Authentication $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.4";
		    } 
		    catch (\Stripe\Error\ApiConnection $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.5";
		    } 
		    catch (\Stripe\Error\Base $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.6";
		    } 
		    catch (Exception $e) {
		      echo $error = "Sorry, we weren't able to authorize your card. You have not been charged.7";
		    }
    	}
    }
}
