...
class cloud_token extends CI_Controller {
	...
	{	
	    $usercode=$this->session->userdata('logged_in_visa')["usercode"];// get user code from session
	    $status=$this->notification_module->check_cloud_token_status($usercode);// check the status of cloud token
	   $status= $status["cloud_token_status"];
	 
	    if($status=="Active"){
	    $data['contain']=$result=$this->notification_module->get_pages_contain('cloud_token_paid');
	    }else{
	           $data['contain']=$result=$this->notification_module->get_pages_contain('cloud_token_free');
	    }
