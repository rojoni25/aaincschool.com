...
class cmspages extends CI_Controller {
	...
	public function p()
	{
		$data['contain']=$this->cmspages_module->get_pages_contain($this->uri->segment(3));
	...
	public function affiliate(){
		...
		if($this->session->userdata['tbl']['current_account']=='Active')
		{
			if(time() < $this->session->userdata['logged_ol_member']['due_time']){
				$data['contain']=$this->cmspages_module->get_pages_contain('affiliate_active');
			}
			else{
				$data['contain']=$this->cmspages_module->get_pages_contain('affiliate_due');
			}
		}
		else{
			$data['contain']=$this->cmspages_module->get_pages_contain('affiliate_free');
		}
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('cmspages_view',$data);
