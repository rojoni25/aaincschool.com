...
class comman_controler extends CI_Controller {
	...
	function get_level_summary(){
		$date_text=isset($_GET['date']) && $_GET['date']!=''?$_GET['date']:'Yesterday';
		$date_for=isset($_GET['date']) && $_GET['date']!=''?$_GET['date']:'yesterday';
		...
		$level_payment		=	$this->ObjM->yesterday_payment_sum_by_level('3by3');
		$level_payment_total=	$this->ObjM->payment_sum_by_level('3by3');
		$level_sum		    =	$this->ObjM->yesterday_payment_sum('3by3');

		$pay_level_one_total=isset($level_payment_total[0]['total'])?$level_payment_total[0]['total']:0;
		$pay_level_two_total=isset($level_payment_total[1]['total'])?$level_payment_total[1]['total']:0;
		$pay_level_three_total=isset($level_payment_total[2]['total'])?$level_payment_total[2]['total']:0;
		
		if(isset($level_payment[0])){
			$pay_level_one=$level_payment[0]['total'];
			$pay_level_one='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/3by3/1">'.$pay_level_one.'$</a>';
		}
		else{$pay_level_one='0.00$';}
		
		if(isset($level_payment[1])){
			$pay_level_two=$level_payment[1]['total'];
			$pay_level_two='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/3by3/2">'.$pay_level_two.'$</a>';
		}
		else{$pay_level_two='0.00$';}
		
		if(isset($level_payment[2])){
			$pay_level_three=$level_payment[2]['total'];
			$pay_level_three='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/3by3/3">'.$pay_level_three.'$</a>';
		}
		...
		$level_payment_today 		=	$this->ObjM->yesterday_payment_sum_by_level('3by3');
		$level_payment_total_today	=	$this->ObjM->payment_sum_by_level('3by3');
		$level_sum_today		    =	$this->ObjM->yesterday_payment_sum('3by3');
		
		
		if(isset($level_payment_today[0])){
			$pay_level_one_today=$level_payment_today[0]['total'];
			$pay_level_one_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/3by3/1">'.$pay_level_one_today.'$</a>';
		}
		else{$pay_level_one_today='0.00$';}
		
		if(isset($level_payment_today[1])){
			$pay_level_two_today=$level_payment_today[1]['total'];
			$pay_level_two_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/3by3/2">'.$pay_level_two_today.'$</a>';
		}
		else{$pay_level_two_today='0.00$';}
		
		if(isset($level_payment_today[2])){
			$pay_level_three_today=$level_payment_today[2]['total'];
			$pay_level_three_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/3by3/3">'.$pay_level_three_today.'$</a>';
		}
		else{$pay_level_three_today='0.00$';}

        // end get todays summery
		...
		if($_REQUEST['tree']=='3')
		{
			$html='<table class="table table-bordered">
					...
					<tr><td>Level-1</td><td>'.$summary[0]['level_one3'].'</td></tr>
					<tr><td>Level-2</td><td>'.$summary[0]['level_two3'].'</td></tr>
					<tr><td>Level-3</td><td>'.$summary[0]['level_three3'].'</td></tr>
					<tr>
						<th>Total</td>
		...
		if($_REQUEST['tree']=='5')
		{
			...
			$level_payment		=	$this->ObjM->yesterday_payment_sum_by_level('5by3');
			$level_payment_total=	$this->ObjM->payment_sum_by_level('5by3');
			$level_sum		    =	$this->ObjM->yesterday_payment_sum('5by3');
			...
			if(isset($level_payment[0])){
				$pay_level_one=$level_payment[0]['total'];
				$pay_level_one='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/5by3/1">'.$pay_level_one.'$</a>';
			}
			else{$pay_level_one='0.00$';}		
			
			if(isset($level_payment[1])){
				$pay_level_two=$level_payment[1]['total'];
				$pay_level_two='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/5by3/2">'.$pay_level_two.'$</a>';
			}
			else{$pay_level_two='0.00$';}		
			
			if(isset($level_payment[2])){
				$pay_level_three=$level_payment[2]['total'];
				$pay_level_three='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/5by3/3">'.$pay_level_three.'$</a>';
			}
			else{$pay_level_three='0.00$';}
			...
			$_GET['date'] = date("d-m-Y");
			$level_payment_today 		=	$this->ObjM->yesterday_payment_sum_by_level('5by3');
			$level_payment_total_today	=	$this->ObjM->payment_sum_by_level('5by3');
			$level_sum_today		    =	$this->ObjM->yesterday_payment_sum('5by3');
			
			
			if(isset($level_payment_today[0])){
				$pay_level_one_today=$level_payment_today[0]['total'];
				$pay_level_one_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/5by3/1">'.$pay_level_one_today.'$</a>';
			}
			else{$pay_level_one_today='0.00$';}
			
			if(isset($level_payment_today[1])){
				$pay_level_two_today=$level_payment_today[1]['total'];
				$pay_level_two_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/5by3/2">'.$pay_level_two_today.'$</a>';
			}
			else{$pay_level_two_today='0.00$';}
			
			if(isset($level_payment_today[2])){
				$pay_level_three_today=$level_payment_today[2]['total'];
				$pay_level_three_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/5by3/3">'.$pay_level_three_today.'$</a>';
			}
			else{$pay_level_three_today='0.00$';}
			...
			$html ='<table class="table table-bordered">
					...
					<td>5x3 Daily('.$date_text.')</td>
					<td>5x3 Daily(Today)</td>
					<td>5x3 Daily Total</td>
					</tr>
					<tr><td>Level-1</td><td>'.$summary[0]['level_one5'].'</td><td>'.$summary[0]['active_level_one5'].'</td><td>'.$pay_level_one.'</td><td>'.$pay_level_one_today.'</td><td>'.$pay_level_one_total.'</td></tr>
					<tr><td>Level-2</td><td>'.$summary[0]['level_two5'].'</td><td>'.$summary[0]['active_level_two5'].'</td><td>'.$pay_level_two.'</td><td>'.$pay_level_two_today.'</td><td>'.$pay_level_two_total.'</td></tr>
					<tr><td>Level-3</td><td>'.$summary[0]['level_three5'].'</td><td>'.$summary[0]['active_level_three5'].'</td><td>'.$pay_level_three.'</td><td>'.$pay_level_three_today.'</td><td>'.$pay_level_three_total.'</td></tr>
					<tr>
						<th>Total</th>
						<th>'.$tot_three_five_member.'</th>
						<th>'.$tot_three_five_member_active.'</th>
						<th>$'.$level_sum[0]['total'].'</th>
						<th>$'.$level_sum_today[0]['total'].'</th>
						<th>$'.($pay_level_one_total+$pay_level_two_total+$pay_level_three_total).'</th>
					</tr>
					</table>';
		}
		if($_REQUEST['tree']=='10')
		{
			...
			$level_payment		=	$this->ObjM->yesterday_payment_sum_by_level('10by3');
			$level_payment_total=	$this->ObjM->payment_sum_by_level('10by3');
			$level_sum		    =	$this->ObjM->yesterday_payment_sum('10by3');
			...
			if(isset($level_payment[0])){
				$pay_level_one=$level_payment[0]['total'];
				$pay_level_one='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/10by3/1">'.$pay_level_one.'$</a>';
			}
			else{$pay_level_one='0.00$';}		
			
			if(isset($level_payment[1])){
				$pay_level_two=$level_payment[1]['total'];
				$pay_level_two='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/10by3/2">'.$pay_level_two.'$</a>';
			}
			else{$pay_level_two='0.00$';}		
			
			if(isset($level_payment[2])){
				$pay_level_three=$level_payment[2]['total'];
				$pay_level_three='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$date_for.'/10by3/3">'.$pay_level_three.'$</a>';
			}
			else{$pay_level_three='0.00$';}
			...
			$_GET['date'] = date("d-m-Y");
			$level_payment_today 		=	$this->ObjM->yesterday_payment_sum_by_level('10by3');
			$level_payment_total_today	=	$this->ObjM->payment_sum_by_level('10by3');
			$level_sum_today		    =	$this->ObjM->yesterday_payment_sum('10by3');
			
			
			if(isset($level_payment_today[0])){
				$pay_level_one_today=$level_payment_today[0]['total'];
				$pay_level_one_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/10by3/1">'.$pay_level_one_today.'$</a>';
			}
			else{$pay_level_one_today='0.00$';}
			
			if(isset($level_payment_today[1])){
				$pay_level_two_today=$level_payment_today[1]['total'];
				$pay_level_two_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/10by3/2">'.$pay_level_two_today.'$</a>';
			}
			else{$pay_level_two_today='0.00$';}
			
			if(isset($level_payment_today[2])){
				$pay_level_three_today=$level_payment_today[2]['total'];
				$pay_level_three_today='<a class="payment_form" href="'.base_url().'index.php/payment_report/get_paid_free_from/'.$_GET['date'].'/10by3/3">'.$pay_level_three_today.'$</a>';
			}
			else{$pay_level_three_today='0.00$';}
			...
			$html .='<table class="table table-bordered">
					...
					<td>People</td>
					<td>NO. of Active Users</td>
					<td>10x3 Daily('.$date_text.')</td>
					<td>10x3 Daily(Today)</td>
					<td>10x3 Daily Total</td>
					</tr>
					<tr><td>Level-1</td><td>'.$summary[0]['level_one10'].'</td><td>'.$summary[0]['active_level_one10'].'</td><td>'.$pay_level_one.'</td><td>'.$pay_level_one_today.'</td><td>'.$pay_level_one_total.'</td></tr>
					<tr><td>Level-2</td><td>'.$summary[0]['level_two10'].'</td><td>'.$summary[0]['active_level_two10'].'</td><td>'.$pay_level_two.'</td><td>'.$pay_level_two_today.'</td><td>'.$pay_level_two_total.'</td></tr>
					<tr><td>Level-3</td><td>'.$summary[0]['level_three10'].'</td><td>'.$summary[0]['active_level_three10'].'</td><td>'.$pay_level_three.'</td><td>'.$pay_level_three_today.'</td><td>'.$pay_level_three_total.'</td></tr>
					<tr>
						<td>Total</td>
	...
	function get_top_banner()
	{
		...
		if($this->session->userdata['logged_ol_member']['status']=='Pending')
		{
			...
			$member_welcome_left = $this->ObjM->cms_by_lable("free-member-welcome-left");
			$left_text = strip_tags($member_welcome_left[0]['textdt']);

			$member_welcome_right = $this->ObjM->cms_by_lable("free-member-welcome-right");
			$right_text = strip_tags($member_welcome_right[0]['textdt']);

			$html.='<div class="banner-inner">
        			<div class="col-md-4"><div class="alert banner-info magin0"><span class="hide_in_mobile">'.$left_text.'</span> </div></div>
        			<div class="col-md-4" style=""><div class="left_side info magin0"><strong></strong> &nbsp;<a href="'.base_url().'index.php/view_friends/invitefriends/"><button class="btn btn btn-primary"><strong>Invite Friends</strong></button></a></div></div>
            		<div style="clear:both;overflow:hidden;"></div>
        		  </div>';
					/*   	 				<div class="span4"><div class="left_side info magin0"><strong>'.$right_text.'</strong> &nbsp;<a href="'.base_url().'index.php/upgrade_membership/view"><button class="btn btn btn-warning"><strong>Upgrade Membership</strong></button></a></div></div>
*/				
			}
			else{
			...
			$member_welcome_left = $this->ObjM->cms_by_lable("paid-member-welcome-left");
			$left_text = strip_tags($member_welcome_left[0]['textdt']);

			$member_welcome_right = $this->ObjM->cms_by_lable("paid-member-welcome-right");
			$right_text = strip_tags($member_welcome_right[0]['textdt']);
		...
		if($this->session->userdata['logged_ol_member']['status']=='Active' && $this->session->userdata['logged_ol_member']['usercode']!='1')
		{
			...
			if($due_day7 < time()){
				...
				$html.='<div class="banner-inner">
            			<div class="col-md-4"><div class="alert banner-info magin0"><strong> <i class="fa fa-check-square"></i>  '.$msg.' </strong>'.$duedt.'</div></div>
            			
            			<div class="col-md-4" style="margin-left: 180px;"><div class="left_side info magin0"><strong></strong> &nbsp;<a href="'.base_url().'index.php/view_friends/invitefriends/"><button class="btn btn btn-primary"><strong>Invite Friends</strong></button></a></div></div>

       	 				
                		<div style="clear:both;overflow:hidden;"></div>
            		  </div>';
					/*<div><div class="left_side info magin0 temp-hide"><a href="'.base_url().'index.php/monthly_payment_active_member/pay"><button class="btn btn btn-primary"><strong>Active Affiliate</strong></button></a></div></div>*/
			}
	...
	function get_daily_payment_free()
	{
			...
			$data['master_sheet']	=	$this->ObjM->get_daily_payment_free();
			...
			$total_wallet = number_format($monthly_total,2) + $payment[0]['3by3daily'] + $payment[0]['5by3daily'] + $payment[0]['10by3daily'] + number_format($coded[0]['tot']*$coded_pay[0]['setting_value'],2) + number_format($coded_match[0]['tot']*$coded_match_pay[0]['setting_value'],2) + number_format($residual[0]['tot']*$residual_pay[0]['setting_value'],2) + number_format($residual_match[0]['tot']*$residual_match_pay[0]['setting_value'],2);
			...
			<li><a href="#"><strong>Total Wallet</strong>: <span>$'.$total_wallet.'<span></a></li>
			<div style="clear:both;overflow:hidden;"></div>
			</ul>';
			// <li><a href="#"><strong>Total Wallet</strong>: <span>$'.$data['master_sheet'][0]['main_balance'].'<span></a></li>
			return $html;
	}
