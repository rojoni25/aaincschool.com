...
class ad_request extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('kdk1/ad_request_model','ObjM',TRUE);
	...
	function request()
	{
		...
		$this->load->view(''.MATRIX_FOLDER.'/admin/ad_member_code_request');
	...
	function access_code_popup($eid)
	{
		...
		$this->load->view(''.MATRIX_FOLDER.'/admin/access_code_popup',$data);	
	}
	...
	function access_code_insert()
	{
		...
		if(isset($chk[0]))
		{
			...
			if($_POST['join_request']=='Y'){
				$this->insert_join_request($result[0]);
			}
	...
	function pending()
	{
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view(''.MATRIX_FOLDER.'/admin/r_matrix_pending_req',$data);
		$this->load->view('comman/footer');	
	}
