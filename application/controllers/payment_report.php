...
class payment_report extends CI_Controller {
	...
	function get_paid_from($time, $report_type){
		if($time=='yesterday'){
	...
	function get_paid_free_from($time, $report_type, $level){
		$usercode=$this->session->userdata['logged_ol_member']['usercode'];
		if($time=='yesterday'){
			$time=strtotime(date('d-m-Y', strtotime('-1 day')));
		} else{
			$time=strtotime($time);
		}
		$arr=array(
		...
		if($this->ObjM->coded_residual=='coded_residual'){
			$result=$this->ObjM->get_paid_from($arr);
		} else{
