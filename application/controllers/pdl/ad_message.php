...
class ad_message extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('pdl/admin/ad_message_model','ObjM',TRUE);
	...
	public function inbox()
	{
		...
		$this->load->view('pdl_admin/ad_message_view',$data);
		...
		for($i=0;$i<count($rt);$i++)
		{
			...
			$html.='<div class="alert noto-div read_status'.$rt[$i]['read_status'].'">
						...
						<ul class="list_um">
							...
							<li><a class="show-pop-event" href="'.base_url().'index.php/pdl/'.$this->uri->rsegment(1).'/popover_form/'.$rt[$i]['id'].'/receive"><i class="icon-share-alt"></i></a></li>
	...
	function compose(){
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('pdl_admin/ad_message_send',$data);
		$this->load->view('comman/footer');	
	...
	function popover_form($eid,$send)
	{
		...
		<form action="'.base_url().'index.php/pdl/'.$this->uri->rsegment(1).'/message_insert" method="POST" id="frm_pay">
