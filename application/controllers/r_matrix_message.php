...
class r_matrix_message extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('r_matrix_message_model','ObjM',TRUE);
	...
	public function inbox()
	{
		...
		$this->load->view('r_matrix/r_matrix_message_view',$data);
		...
		for($i=0;$i<count($rt);$i++)
		{
			...
			$html.='<div class="alert noto-div read_status'.$rt[$i]['read_status'].'">
					...
					<ul class="list_um">
						...
						<li><a class="show-pop-event" href="'.base_url().'index.php/'.$this->uri->segment(1).'/popover_form/'.$rt[$i]['id'].'/receive"><i class="icon-share-alt"></i></a></li>
		...
		for($i=0;$i<count($rt);$i++)
		{
			...
			$html.='<div class="alert noto-div">
					...
					<ul class="list_um">
						...
						<li><a class="show-pop-event" href="'.base_url().'index.php/'.$this->uri->segment(1).'/popover_form/'.$rt[$i]['id'].'/send"><i class="icon-share-alt"></i></a></li>
	...
	function compose(){
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('r_matrix/r_matrix_message_send',$data);
		$this->load->view('comman/footer');	
	...
	function message_insert(){
		...
		{	
			...
			$this->ObjM->addItem($data,'rm_message');
				
			$json['vali']		=	'true';
			$json['msg']		=	'<h4>Successfully Send</h4>';
			
			if($_POST['send_form']=='compose'){
				header('Location: '.base_url().'index.php/'.$this->uri->segment(1).'/outbox/');
				exit;
			}	
