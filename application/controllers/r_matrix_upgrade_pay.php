...
class r_matrix_upgrade_pay extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('r_matrix_upgrade_pay_model','ObjM',TRUE);
	...
	public function pif_remaining($eid)
	{
		...
		$this->load->view('r_matrix/r_matrix_upgrade_pay_view',$data);
	...
	public function pif_send_report($eid)
	{
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('r_matrix/r_matrix_pif_send_report',$data);
		$this->load->view('comman/footer');
	}
	...
	function payment_insert(){
		...
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			if(isset($result[0])){
				...
				$this->ObjM->addItem($data,'rm_member_upgrade_pay');
	...
	protected function pif_email_to_admin($result){
			...
			$message = str_replace("[fname]",$result['name'],$message);
