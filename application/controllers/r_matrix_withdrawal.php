...
class r_matrix_withdrawal extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('r_matrix_withdrawal_model','ObjM',TRUE);
	...
	public function request($eid)
	{
		...
		$this->load->view('r_matrix/r_matrix_withdrawal_view',$data);
	...
	function listing(){
		...
		for($i=0;$i<count($result);$i++){
			...
			$btn.='<a class="delete_request" href="'.base_url().'index.php/'.$this->uri->segment(1).'/delete_request/'.$result[$i]['req_id'].'" class="show-pop-event"><span class="label label-important">Delete</span></a>';
			$btn.='<a class="notification_link" href="'.base_url().'index.php/r_matrix_notification/popup/'.$result[$i]['usercode'].'"><i class="icon-bell-alt"></i></a>';
		...
		if(isset($result[0])){
			...
			if($pay['coin_balance'] < $amount ){
				...
				exit;
					...
					<form action="'.base_url().'index.php/'.$this->uri->segment(1).'/payment_insert" method="POST" id="frm_pay">
					...
					<tr><td>Current Coin Wallet</td><td></td><td>$'.number_format($pay['coin_balance'],2).'</td></tr>
		...
		{	
			...
			if(isset($result[0])){
				...
				if($pay['coin_balance'] >= $amount ){
					...
					$data=array();
					$data['status']		=	'A';
					$this->ObjM->update($data,'rm_member_withdrawal_request','req_id',$result[0]['req_id']);
	...
	function delete_request($eid){
		$data=array();
		$data['status']		=	'C';
		$this->ObjM->update($data,'rm_member_withdrawal_request','req_id',$eid);	
	}
