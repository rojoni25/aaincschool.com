...
class residual_match_tree extends CI_Controller {

	function __construct()
 	{
   		parent::__construct(); 
		if(!$this->session->userdata('logged_ol_member')){header('Location: '.base_url().'index.php/login');exit;} 
		$this->load->model('residual_match_tree_model','',TRUE);
	...
	{
		$result=$this->residual_match_tree_model->get_all_referral($this->session->userdata['logged_ol_member']['usercode']);
		...
		for($i=0;$i<count($result);$i++){
			$html.='<li class="coded_li_click" level="1" posi="'.$i.'" main_margin="0" usercode="'.$result[$i]['usercode'].'">
						...
						<img src="'.base_url().'/asset/images/multitree/residual_match.png">
	...
	function get_next_level(){
		...
		$result=$this->residual_match_tree_model->get_all_referral_next($usercode);
