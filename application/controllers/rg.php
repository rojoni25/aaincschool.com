...
class rg extends CI_Controller {
	...
	function send_refer_click($user){
		...
		$this->send_notification($refer_data[0]['usercode'],$noti_msg, $user[0]['usercode']);
	}
	...
	function insertrecord(){
		...
		if($this->input->post('referralid')!=''){
			if ($this->input->server('REQUEST_METHOD') === 'POST')
			{
				...
				$this->enroll_free_wallet($usercode,$referralid);
	...
	public function send_email_friend($usercode, $firstname, $lastname){
        ...
        $message = '<br>Your team is growing  <br> Name : '.$fullname.' <br> Referred By : '.$referalname.'<br>Congrats and many blessings.<br>';
        ...
        $noti_msg='New Team Member '.$fullname.' Signup Successfully';
        ...
        for($i=0;$i<count($arr);$i++){
            // $this->email->to($arr[$i]);
            // $this->email->send();
            sendemail(FROM_EMAIL,'Congratulations New team member',$arr[$i],$message);
...
//-------------All Function for Add Multiplier Free Tree.------------------------//
	...
	function wallet_cron(){
		$allmem = $this->rg_model->ref_all_member();
		foreach ($allmem as $mem) {
			$usercode = $mem['usercode'];
			$referralid = $mem['referralid_free'];
			echo $usercode;
			$this->enroll_free_wallet($usercode,$referralid);
			echo '<br>';
		}
	}
	function enroll_free_wallet($usercode,$referralid){
		$member_email	=	$this->rg_model->get_member_detail_by_code($usercode);
		$upgradelevel  = $this->rg_model->get_upgrade_level($usercode);
        ...
        $this->rg_model->addItem($data,'tbl_free_payment_wallet');

        //
        //
        $referalamt =  $upgradelevel['level_referal_commision'];
        $data = array();
        $data['receiverid'] =  $referralid;
        $data['senderid'] =  $usercode;
        $data['amount'] =  $referalamt;
        $data['plan'] =  $levelcode;
        $data['date'] =  date('Y-m-d H:i:s');
        $this->rg_model->addItem($data,'tbl_free_visible_wallet');
        //
        //
        $uplineid = $this->rg_model->get_twoleveluplineid($usercode);
        ...
        $data['plan'] =  $levelcode;
        $data['date'] =  date('Y-m-d H:i:s');
        $this->rg_model->addItem($data,'tbl_free_hidden_wallet');

        //Level Update
        $level['current_free_level']	=	$member_email[0]['current_free_level']+1;
		$this->rg_model->update($level,'membermaster','usercode',$usercode);

		//Upgrade upline level
		$nextupgradelevel  = $this->rg_model->get_upgrade_level($uplineid);
		$nextlevelamt =  $nextupgradelevel['level_amount'];
		$curlevel =  $nextupgradelevel['level_code']-1;
		if($curlevel>0){
			$curentwalletamt = $this->rg_model->getfreememberlevelwallet($uplineid,$curlevel);
			if($nextlevelamt==$curentwalletamt){
				$memberdata	=	$this->rg_model->get_member_detail_by_code($uplineid);
				$usercode=$uplineid;
				$referralid=$memberdata[0]['referralid_free'];
				$this->enroll_free_wallet($usercode,$referralid);
				/*
				$upgradelevel  = $this->rg_model->get_upgrade_level($usercode);
		        $levelcode =  $upgradelevel['level_code'];
		        ...
		        //Level Update
		        $level['current_free_level']	=	$memberdata[0]['current_free_level']+1;
				$this->rg_model->update($level,'membermaster','usercode',$usercode);
				*/
			}
		}
	...
	function reg_cron(){
		...
		//echo $referralid = $this->getreferalid();
		//die('111');
		$total = 28;
		for ($i=1; $i < $total ; $i++) { 
			$totalmem = $this->db->count_all_results('membermaster');
			$no = $totalmem + 1;
    		//$username = 'number'.$no;
    		$username = 'member'.$no;
    		$password = random_string('numeric', 6);
			...
			$referralid = $this->getreferalid();
    		
    		$data['fname']			 =	'f'.$no;
			$data['lname']			 =	'l'.$no;
			$data['username']		 =	$username;
			$data['password']		 =	$password;
			$data['emailid']		 =	$no.'@gmail.com';
			$data['mobileno']		 =	'111111111111';	
			$data['referralid'] 	 =	$referralid;
			$data['referralid_free'] =	$referralid;
			$data['add_time'] 		 =	strtotime(date('d-m-Y'));
	...
	public function getreferalid(){
		$this->load->database();
		$query = $this->db->query("SELECT * FROM `membermaster` WHERE referralid_free!=0 GROUP BY `referralid_free` HAVING COUNT(*) < 3 ORDER BY `referralid_free` ASC LIMIT 0,1");
	    $row = $query->row_array();
	    $refid = $row['referralid_free'];
	    if($refid>0){
		    return $refid;
	    }else{
	    	$this->load->database();
	    	$query = $this->db->query("SELECT * FROM `membermaster` WHERE usercode NOT IN (SELECT referralid_free FROM membermaster) ORDER BY usercode LIMIT 0,1");
		    $row = $query->row_array();
		    $refid = $row['usercode'];
		    return $refid;
	    }
	...
	//
	public function reverse_reg(){
		//error_reporting(E_ALL);
		$uArr = $this->rg_model->ref_all_member();
		echo count($uArr);
		foreach ($uArr as $user) {
			if($user['usercode']!=2){
				$this->entry_in_reverse_tree(2,$user['usercode']);
			}
		}
		//$this->entry_in_reverse_tree(2,18);
	}
	protected function entry_in_reverse_tree($pid,$uid){
			...
			$data['usercode']			=	$uid;
				...
				// add to reverse matrix 2x3 
// 			$set_arr=array('field' => 'uplingmember2_3','position' => 2,'side' => 'side_2_3');
// 			$this->first_lavel_reverse_set($pid,$set_arr,$uid);
// 			$data['uplingmember2_3']	=	$this->upling_reverse_user;
// 			$data['side_2_3']			=	$this->upling_reverse_posi;
			...
			// add reverse matrix nodes to database
			$this->rg_model->addItem($data,'member_node_reverse_free');
			
			// ==================================Add Commision to Free Reverse Wallet================================================================
		    $com2x3=	$this->rg_model->get_setting_value_by_lable('free_rev_wallet_2x3');
		    $com3x3=	$this->rg_model->get_setting_value_by_lable('free_rev_wallet_3x3');
			$com5x3	=	$this->rg_model->get_setting_value_by_lable('free_rev_wallet_5x3');// get commission value from setting
			$com10x3	=	$this->rg_model->get_setting_value_by_lable('free_rev_wallet_10x3');
			
			$rdata['amt_2_3'] = $com2x3[0]['setting_value'];
			$rdata['remain_2_3'] = $com2x3[0]['setting_value'];
			$rdata['amt_3_3'] = $com3x3[0]['setting_value'];
			$rdata['remain_3_3'] = $com3x3[0]['setting_value'];
			...
			$rdata['usercode'] = $uid;
			$this->rg_model->addItem($rdata,'tbl_free_reverse_wallet');
			return $data;
		
	}
	protected function first_lavel_reverse_set($pid,$type,$uid){
		
		$result	=	$this->rg_model->get_usercode_by_reverse_tree($pid,$type['field']);
		//print_r($result);
		//die();
		///level One Setting///
		if(count($result)<$type['position']){
	...
	protected function tree_check_reverse_postion($uid,$arr_mem, $type){
		...
		//for($pos=0;$pos<$type['position'];$pos++){
			...
			for($i=0;$i<count($arr_mem);$i++){
				$result = $this->rg_model->get_reverse_tree_by_usercode($arr_mem[$i]);
	...
	public function change_parent_tree($uid,$pid,$type){
		...
		$result	=	$this->rg_model->get_usercode_by_reverse_tree($uid,$type['field']);
		if(count($result)>0){
			for($i=0; $i < count($result); $i++) {
				$newpos = $i+2; 
	...
	protected function swap_position($usercode,$olduid,$newpos,$type){
		...
		$this->db->where($array); 
		$this->db->update('member_node_reverse_free', $data); 
	}
