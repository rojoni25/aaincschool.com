...
//-------------All Function for Add Multiplier Free Tree.------------------------//
	...
	public function reverse_reg(){
    	...
    	$query = $this -> db -> get();
    	$lArr = $query->row_array();
    	echo $totalrow = $query->num_rows();
    	$nextrow = $totalrow + 1;
		$this -> db -> select('*');
   		$this -> db -> from('membermaster');
   		$this -> db -> where('status !=', 'Delete');
   		$this -> db -> limit(50,$nextrow);
	...
	protected function entry_in_reverse_tree($pid,$uid){
			...
			$commission2	=	$this->rg_model->get_setting_value_by_lable('virtual_balance_level2');
			$commission3	=	$this->rg_model->get_setting_value_by_lable('virtual_balance_level3');
			$rdata['amt_5_3'] = $commission2[0]['setting_value'];
			$rdata['amt_10_3'] = $commission3[0]['setting_value'];
			$rdata['remain_5_3'] = $commission2[0]['setting_value'];
			$rdata['remain_10_3'] = $commission3[0]['setting_value'];
			$rdata['usercode'] = $uid;
