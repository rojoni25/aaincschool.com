...
class rm_martix extends CI_Controller {

	function __construct()
 	{
   		parent::__construct(); 
		if(!$this->session->userdata('logged_ol_member')){header('Location: '.base_url().'index.php/login');exit;} 
		$this->load->model('rm_martix_module','ObjM',TRUE);
	...
	public function view()
	{
		...
		$check_sdk	=	$this->ObjM->get_sdk_code();
	...
	protected function r_join_view(){
		...
		$this->load->view('r_matrix_member/rm_martix_join',$data);
	...
	protected function r_joined_member_view(){
		...
		$data['cms']		  =	$this->ObjM->get_cms_page('r_matrix');
		...
		$this->load->view('comman/topheader');
		$this->load->view('comman/header');
		$this->load->view('r_matrix_member/rm_martix_home',$data);
		$this->load->view('comman/footer');
	}
	...
	function insert_request_pop(){
		$html='<div class="pop-div-main">
			<form action="'.base_url().'index.php/'.$this->uri->segment(1).'/insert_request" method="POST" id="send_request_join">
			<table class="table table-striped table-bordered dataTable">
			<tr><td colspan="3"><h4>Send Request To Join R-Matrix </h4></td></tr>
