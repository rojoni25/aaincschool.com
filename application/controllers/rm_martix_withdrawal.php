...
class rm_martix_withdrawal extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('rm_martix_withdrawal_module','ObjM',TRUE);
	...
	public function view()
	{
		...
		$this->load->view('r_matrix_member/rm_martix_withdrawal_add',$data);
	...
	function insert_request(){
		if ($this->input->server('REQUEST_METHOD') === 'POST')
		{
			...
			$pending_request	=	$this->ObjM->get_pending_request();
			
			if(isset($pending_request[0]))
			{
				$arr=array('class'=>'alert alert-error','text'=>'Error : One Request Already Pending','sign'=>'icon-exclamation-sign');
			...
			if($amount > $pay['coin_balance']){
				...
				$data['status']		=	'P';			
				$id=$this->ObjM->addItem($data,'rm_member_withdrawal_request');
