...
class rm_pif_friend extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('rm_pif_friend_module','ObjM',TRUE);
	...
	public function view()
	{
		...
		$this->load->view('r_matrix_member/rm_pif_friend_view',$data);
	...
	public function extra_position_popup()
	{
		...
		if($pay['coin_balance']>=110){
			...
			<form action="'.base_url().'index.php/'.$this->uri->segment(1).'/add_request" method="POST" id="frm_request">
	...
	protected function pif_email_to_admin(){
			...
			$message = get_email_cms_page_master('pif_friend')->result()[0]->textdt;
