...
class three_by_three extends CI_Controller {
	...
	function get_tree($code=''){
		...
		$tree_arr=array();
		$url=''.base_url().'index.php/three_by_three/tree/';
		$top_nm=$topnode[0]['fname'].' '.$topnode[0]['lname'];
		$tree_arr['top_img']="<img src='".base_url()."/asset/images/multitree/".$img."' title='".$name."' />";
		...
		for($i=0;$i<count($lev3);$i++){
			...
			if(isset($rg[0])){
				...
				$tree_arr['lev3img'.$p]="<a href='".$url."".$rg[0]['usercode']."'><img src='".base_url()."asset/images/multitree/".$img."' title='' /></a>";		
			}
			else{
				$tree_arr['lev3nm'.$p]="";
			...
			if(isset($rg[1])){
				...
				$tree_arr['lev3img'.$p]="<a href='".$url."".$rg[1]['usercode']."'><img src='".base_url()."asset/images/multitree/".$img."' title='' /></a>";		
			}
			else{
				$tree_arr['lev3nm'.$p]="";
				$tree_arr['lev3img'.$p]="<img src='".base_url()."asset/images/multitree/empty.png' title='' />";		
			...
			if(isset($rg[2])){
				...
				$tree_arr['lev3img'.$p]="<a href='".$url."".$rg[2]['usercode']."'><img src='".base_url()."asset/images/multitree/".$img."' title='' /></a>";		
			}
			else{
				$tree_arr['lev3nm'.$p]="";
				$tree_arr['lev3img'.$p]="<img src='".base_url()."asset/images/multitree/empty.png' title='' />";		
	...
	function get_dropdown_createTree($array, $currentParent, $currLevel = 0, $prevLevel = -1){
		foreach ($array as $categoryId => $category) 
		{
			...
			echo '<li class="'.$currLevel.'-'.$prevLevel.'"><label for="subfolder2">'.$category['name'].'&nbsp;<a href="#" value="'.$category['id'].'" class="clsmemdt">
					<i class="fa fa-arrow-right"></i></a></label><input type="checkbox" id="subfolder2"/>';
	...
	function drow_breadcrumb_level($code){
		...
		while(1){
			...
			$user['bread'][]=$result;
			if($result[0]['uplingmember3_3']==$this->session->userdata['logged_ol_member']['usercode']){
