...
class withdrawal extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('vma_ad/withdrawal_model','ObjM',TRUE);
	...
	public function request()
	{
		...
		$this->load->view('vma_ad/withdrawal_list',$data);
				...
				<td>
					<div class="btn-group">
							...
							<ul class="dropdown-menu">
								<li><a href="'.vma_ad().$this->uri->rsegment(1).'/delete_request/'.$result[$i]['id'].'">Delete</a></li>
								<li><a href="'.vma_ad().$this->uri->rsegment(1).'/approve_request/'.$result[$i]['id'].'">Approve</a></li>
							</ul>
						</div>
				</td>	
			</tr>';	
	...
	function delete_request($eid){
		...
		header('Location: '.vma_ad().$this->uri->rsegment(1).'/request');
		exit;
	}
