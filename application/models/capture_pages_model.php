...
Class capture_pages_model extends CI_Model
{
	...
	function get_admin_email()
 	...
 	function get_upgrade_page_contain(){
		$this -> db -> select('*');
   		$this -> db -> from('cms_pages_master');
   		$this -> db -> where('status !=','Delete');
		if($this->session->userdata['tbl']['current_account']=='Active'){
			$this -> db -> where('pagelable','capture_page_upgrade');
		}
    ...
    //=======================update capture pages subscription id========================
    function add_subscription_id($usercode,$subid){
        $date = date('Y-m-d');
        $this->db->set('subscription_id',$subid);
        $this->db->set('subscription_date',$date);
			$this->db->where('usercode', $usercode);
			$this->db->update('membermaster');
    }
    	//----------------Activate Subscription ----------
    function activate_subscription($usercode)
 	{	
