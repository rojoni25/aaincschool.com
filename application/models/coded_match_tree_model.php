<?php
Class coded_match_tree_model extends CI_Model
{
	...
	function __construct()
 	...
 	{	
		...
		$this -> db -> where(''.$this->coded_residual.'.type','coded_match');
	...
	function get_all_referral_next($eid)
	{
		...
		$this -> db -> where('usercode in(select usercode from '.$this->coded_residual.' where usercode_by="'.$this->session->userdata['logged_ol_member']['usercode'].'" and type="coded_match")');
    	$query = $this -> db -> get();
    	$the_content = $query->result_array();
    	return $the_content;
	}
