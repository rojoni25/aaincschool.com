...
Class comman_controler_model extends CI_Model
{
	...
	{
		// echo $this->member_level_track_master;
		$this -> db -> select('*');
	...
	function yesterday_payment_sum_by_level($field){
	...
	function payment_sum_by_level($field){
		if(isset($_GET['date']) && $_GET['date']!=''){
			$timestamp=strtotime($_GET['date']);
		} else{
			$today_stam = strtotime(date('d-m-Y'));
		...
		$from_time=strtotime(date('Y-m-01', $timestamp));
		$to_time=strtotime(date('Y-m-t', $timestamp));
		...
		$this -> db -> where('usercode', ''.$this->session->userdata['logged_ol_member']['usercode'].'');
		//$this -> db -> where('timestm', ''.$timestamp.'');
		$this -> db -> where('timestm >', ''.$from_time.'');
		$this -> db -> where('timestm <', ''.$to_time.'');
	...
	function yesterday_payment_sum($field)
	{
		if(isset($_GET['date']) && $_GET['date']!=''){
			$timestamp=strtotime($_GET['date']);
		} else{
