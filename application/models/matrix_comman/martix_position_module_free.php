<?php
Class martix_position_module_free extends CI_Model
{
	...
	function get_position_member_count($eid){
			...
			t4.idcode  AS lev3
			FROM ".MATRIX_TABLE_PRE."matrix_free AS t1
			LEFT JOIN ".MATRIX_TABLE_PRE."matrix_free AS t2 ON t2.upling_id = t1.idcode
			LEFT JOIN ".MATRIX_TABLE_PRE."matrix_free AS t3 ON t3.upling_id = t2.idcode
			LEFT JOIN ".MATRIX_TABLE_PRE."matrix_free AS t4 ON t4.upling_id = t3.idcode
			WHERE t1.idcode = '".$eid."'";
	...
	function get_multi_position_detail($eid)
	{
		...
		$this -> db -> join(''.MATRIX_TABLE_PRE.'member_payment_free pay_coin','pay_coin.position = '.MATRIX_TABLE_PRE.'matrix_free.idcode and pay_coin.wallet_type="COIN"','left');
		
		
		$this -> db -> where(''.MATRIX_TABLE_PRE.'matrix_free.usercode',''.$this->session->userdata['logged_ol_member']['usercode'].'');
		$this -> db -> order_by(''.MATRIX_TABLE_PRE.'matrix_free.idcode','asc');
		$query = $this -> db -> get();
    	$the_content = $query->result_array();
    	return $the_content;
	...
	function get_payment_sum_by_type($wallet_type)
	{
		$this -> db -> select('SUM(amount) as tot');
   		$this -> db -> from(''.MATRIX_TABLE_PRE.'member_payment_free');
		$this -> db -> where('usercode',''.$this->session->userdata['logged_ol_member']['usercode'].'');
	...
	function get_multi_postion($id){
		...
		$this -> db -> select(''.MATRIX_TABLE_PRE.'matrix_free.*',FALSE);
