...
Class r_matrix_member_model extends CI_Model
{
	...
	function cycle_report(){
		$this -> db -> select('rm_member_payment.*');
		...
		$this -> db -> join('membermaster','membermaster.usercode = rm_member_payment.usercode','left');
   		$this -> db -> from('rm_member_payment');
   		$this -> db -> where('rm_member_payment.wallet_type','COIN');
		$this -> db -> order_by('rm_member_payment.id', 'asc');
		$query = $this -> db -> get();
    	$the_content = $query->result_array();
    	return $the_content;
	}
	
	function recycler_manual_pay(){
		
		$sQuery='SELECT 
		COUNT(tbl3.idcode) as tot,
		rm_matrix.idcode,
		rm_matrix.usercode,
		CONCAT(membermaster.fname," ",membermaster.lname) as name
		FROM (rm_matrix) 
		LEFT JOIN membermaster 	 ON membermaster.usercode = rm_matrix.usercode 
		LEFT JOIN rm_matrix tbl1 ON tbl1.upling_id = rm_matrix.idcode 
		LEFT JOIN rm_matrix tbl2 ON tbl2.upling_id = tbl1.idcode 
		LEFT JOIN rm_matrix tbl3 ON tbl3.upling_id = tbl2.idcode
		WHERE (rm_matrix.idcode NOT IN (select position from rm_member_payment where wallet_type="COIN"))
		GROUP BY  rm_matrix.idcode
		HAVING (COUNT(tbl3.idcode)=8)';
