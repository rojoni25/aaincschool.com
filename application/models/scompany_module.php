<?php
Class scompany_module extends CI_Model
{
	...
	function check_code_intered(){
		$this -> db -> select('*');
   		$this -> db -> from('rm_kdk_code');
		$this -> db -> where('usercode', ''.$this->session->userdata['logged_ol_member']['usercode'].'');
		$this -> db -> where('usercode IN (select usercode from rm_kdk_code_use where usercode ="'.$this->session->userdata['logged_ol_member']['usercode'].'")');
   		$query = $this -> db -> get();
		$the_content = $query->result_array();
		return $the_content;
	}
	
	function check_kdk_code_request(){
		$this -> db -> select('count(*) as tot');
