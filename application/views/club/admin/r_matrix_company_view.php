...
<div class="row-fluid ">
  <div class="span12">
    <div class="primary-head">
      <h3 class="page-header"><?=MATRIX_LLB?> Criptos & Adsacty
      	<a href="<?=MATRIX_BASE?><?=$this->uri->rsegment(1)?>/addnew/add" class="pull-right">
        	<button class="btn btn-success"><strong>Add New </strong></button>
        </a>
      </h3>
    </div>
    <ul class="breadcrumb">
	...
	<table class="table table-striped table-bordered dataTable" id="data-table">
            ...
            <tbody>
            	<?php for($i=0;$i<count($result);$i++){
                    ...
                    <td><?=ucfirst(strtolower($result[$i]['section']))?></td>
                    <td><?=date('M d, Y',strtotime($result[$i]['create_date']))?></td>
                    <th><a href="<?=MATRIX_BASE?><?=$this->uri->rsegment(1)?>/addnew/Edit/<?=$result[$i]['company_code']?>"><span class="label label-success">Edit</span></a></th>
