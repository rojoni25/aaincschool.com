<?php
	...
	}else if($this->uri->segment(1)=="cloud_token"){
	    $main_cloudtoken="active";
	}else if($this->uri->segment(1)=="cloud_token"){
	    $main_trainmoney="active";
	}
?>
			...
			<div class="sb2-1">
				...
				<div class="sb2-13">
					...
					?>
						...
						<li class="<?=$sub_unilevel?>"><a href="<?php echo base_url();?>index.php/view_friends" class="collapsible-header menu-active <?=$sub_unilevel?>"><i class="fa fa-th" aria-hidden="true"></i> View  Leads </a> </li>
						<?php
			            	$loginusercode = $this->session->userdata['logged_ol_member']['usercode'];
			            	$referralcount = countreferral($loginusercode);
			            ?>	
						<li class="<?=$sub_unilevel?>"><a href="<?php echo base_url();?>index.php/unilevel" class="collapsible-header menu-active <?=$sub_unilevel?>"><i class="fa fa-desktop" aria-hidden="true"></i> AAINC  Unilevel (<?=$referralcount?>)</a> </li>

						<li class="<?=$sub_matrix?>"><a href="<?php echo base_url();?>index.php/three_by_three" class="collapsible-header menu-active <?=$sub_matrix?>"><i class="fa fa-users" aria-hidden="true"></i> Prelaunch Matrix</a> </li>
						<li class="<?=$sub_fb?>"><a href="<?php echo base_url();?>index.php/fb_ad_marketing_plan" class="collapsible-header menu-active <?=$sub_matrix?>"><i class="fa fa-th-large" aria-hidden="true"></i> Pro Marketers</a> </li>
					<?	
					//}
					?>
				...
				<!---		<li class="<?=$main_financemenu?>"><a href="javascript:void(0)" class="collapsible-header <?=$main_financemenu?>"><i class="fa fa-briefcase" aria-hidden="true"></i>Finance</a>
							<div class="collapsible-body left-sub-menu">
								<ul>
									...
									<li class=""><a href="<?=base_url()?>index.php/withdrawal_request"><i class="fa fa-list-alt"></i>Withdrawal Request</a></li>
					...
					?>
				        ...
				        ?>
							...
							<li class="<?=$main_productmenu?>"><a href="<?php if(getSubscriptionStatus($this->session->userdata['logged_ol_member']['usercode'])){echo base_url()."index.php/capture_pages/page_thum_list/Add\"";}else{echo base_url()."index.php/capture_pages/upgrade\"";}?> class="collapsible-header <?=$main_productmenu?>"><i class="fa fa-newspaper-o" aria-hidden="true"></i>Capture Pages </a></li>						

			        	<?
				    	}else{
				        ?>
				         <!---------------------------Check whether the user is a capture pages subscriber or not-------------------------->
	                    <li class="<?=$main_productmenu?>"><a href="<?php if(getSubscriptionStatus($this->session->userdata['logged_ol_member']['usercode']))
	                    {
	                        echo base_url()."index.php/capture_pages/page_thum_list/Add\"";
	                        
	                    }
	                    else{
	                    echo base_url()."index.php/capture_pages/upgrade\"";
	                    
	                    }?> class="collapsible-header <?=$main_productmenu?>"><i class="fa fa-newspaper-o" aria-hidden="true"></i>Capture Pages </a></li>						
	                    <?
						}
						?>
						<li class="<?=$main_trainmoney?>"><a href="<?php echo base_url()?>index.php/opportunity/page/business" class="collapsible-header <?=$main_trainmoney?>"><i class="fa fa-money" aria-hidden="true"></i>Money Train</a></li>
						...
						<li class="<?=$main_cloudtoken?>"><a href="<?php echo base_url();?>index.php/cloud_token" class="collapsible-header <?=$main_cloudtoken?>"><img src="<?=base_url('asset/asset_public/images/cloud-token-btn-xsmall.png');?>">  Cloud Token </a></li>
					<?	
					//}
					?>	
					</ul>
