...
<div class="row-fluid">
  <div class="span12">
    <h3 class="page-header"><?=MATRIX_LLB?> Request Position 
    	
        <div class="pull-right">
        	<a href="<?=base_url()?>index.php/monthly_payment_active_member/kdk1pay" class="temp-hide"> <button class="btn btn-success btncustom" type="button">Online Payment</button> </a>
        	<a href="<?=MATRIX_BASE?><?=$this->uri->rsegment(1)?>/extra_position_popup" class="show-pop-event"><button class="btn btn-warning btncustom" type="button">Request Send For Extra Position</button></a>
        	<a href="<?=MATRIX_BASE?>member/dashboard/"> <button class="btn btn-danger btncustom" type="button">Dashboard</button></a>
        </div>
      ...
      <tbody>
        <?php for($i=0;$i<count($accept_result);$i++){
				...
				echo '<tr>
						...
						<th>$'.number_format($accept_result[$i]['credit'],2).'</th>
						<th>$'.number_format($accept_result[$i]['debit'],2).'</th>
