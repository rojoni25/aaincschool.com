<script>
	...
	$(document).on('change','#current_panel',function(e){
		...
		var url='<?=base_url()?>index.php/pdl/<?=$this->uri->rsegment(1)?>/'+value;
		
		window.location.href = url;

	});
	
	$(document).on('submit','#frm_send',function(e){
    ...
    <div class="primary-head">
      <h3 class="page-header">
        ...
        <select id="current_panel">
          <?php
                	$sel_1=($this->uri->rsegment(2)=='inbox') ? "selected='selected'" : "";
					$sel_2=($this->uri->rsegment(2)=='outbox') ? "selected='selected'" : "";
					$sel_3=($this->uri->rsegment(2)=='compose') ? "selected='selected'" : "";
				?>
...
<div class="row-fluid">
  <div class="span12 membertable">
    <form action="<?=base_url()?>index.php/pdl/<?=$this->uri->rsegment(1)?>/message_insert" method="post" id="frm_send">
      ...
      <table class="stat-table table table-stats table-striped table-sortable table-bordered">
        <tr>
          ...
          <td width="1%"></td>
          <td width="80%"><textarea name="txtmsg" id="txtmsg" placeholder="Enter Message"></textarea></td>
        </tr>
        <tr>
          <td></td>
          <td></td>
