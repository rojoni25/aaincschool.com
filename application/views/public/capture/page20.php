...
<?php }?>
 ...
 <header class="clearfix" style="margin-left:10px; ">
...
<div class="container">
  ...
  <div class="textbody" id="textbody">
        ...
        <form class="form-signin" method="post" action="<?php echo base_url();?>index.php/rg/insertrecord">
        	...
        	<p><input type="text" name="mobileno" id="mobileno" placeholder="Mobile Number" class="txt1" title="Mobile Number">
        	</p>
             <?php if(isset($ref[0])){ ?>
...
.bodyvideo{
	width: <?php echo get_hw_videoframe($result[0]['video_frame'])['width'];?>;
	height: <?php echo get_hw_videoframe($result[0]['video_frame'])['height'];?>;
	position:absolute;
	top:158px;
	left:13%;
	background: url(<?=base_url()?>asset/capturepages/<?php echo $result[0]['video_frame'];?>.png) no-repeat center center;
	background-position: center;
    background-size: cover;
}
.videoinner{
	<?php echo get_hw_videoframe_inner($result[0]['video_frame']);?>
}
.formmain {
	position: absolute;
	top: 0px;
