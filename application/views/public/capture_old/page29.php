...
<?php
	...
	$autoplay_state = $result[0]['page_bg_video_autoplay_2'];
      	
      	if($result[0]['page_bg_video']!=''){
			$autoplay_state = $result[0]['page_bg_video_autoplay_2'] == 'Y' ? Y : N;
			if (strpos($result[0]['page_bg_video'], 'youtube') !== false){
	  ...
	  if($result[0]['video_url1']!=''){
	  	$autoplay_state = $result[0]['page_bg_video_autoplay'] == 'Y' ? Y : N;
			if (strpos($result[0]['video_url1'], 'youtube') !== false){
				echo '<iframe width="100%" height="100%" src="'.convertYoutube($result[0]['video_url1'], N, $autoplay_state).'" frameborder="0" allowfullscreen></iframe>';
			}
			else{
