...
<?php if($this->session->flashdata('msg_show')!='') { ?>
<div class="alert alert-success">
	<strong><?=$this->session->flashdata('msg_show')?></strong>
</div>
<?php } ?>
<div class="row-fluid ">
  <div class="span12">
    <div class="primary-head">
      <h3 class="page-header">R-Matrix Member
      	<a href="<?=base_url()?>index.php/r_matrix_member/view" class="pull-right"><span class="label label-success">Member List</span></a>
      </h3>
    </div>
    <ul class="breadcrumb">
...
<div class="row-fluid">
  <div class="span6">
    <div class="page-header">Coin Wallet</div>
    <table class="table">
      <tbody>
        <tr>
          <th width="25%">Coin Wallet</th>
          <td width="1%">:</td>
          <td width="74%">$
    ...
    <table class="table">
      <tbody>
        <tr>
          ...
          <td>:</td>
          <td>$
            <?=number_format($payment['rm_pay'],2)?></td>
        </tr>
        <tr>
          <th>RM Withdrawal</th>
          <td>:</td>
          <td>$
            <?=number_format($payment['rm_withdrawal'],2)?></td>
        </tr>
        <tr>
          <th>RM Balance</th>
          <th>:</th>
          <th>$
            <?=number_format($payment['rm_balance'],2)?></th>
        </tr>
      </tbody>
    </table>
  </div>
    ...
    <table class="table table-striped table-bordered">
      ...
      <thead>
        <?php for($i=0;$i<count($multi_position);$i++){
					$btn='';
					if($multi_position[$i]['total']==14){
						if((int)$multi_position[$i]['coin']==0){
							$btn='&nbsp;&nbsp;<a href="'.base_url().'index.php/r_matrix/manual_payment/'.$multi_position[$i]['idcode'].'"><span class="label label-important">Manual Pay</span></a>';
						}
					}
					$position=$i+1;
					echo '<tr class="tot_m_'.$multi_position[$i]['total'].'">
							...
							<th><a href="'.base_url().'index.php/r_matrix_tree/view/'.$multi_position[$i]['idcode'].'"><span class="label label-info">Tree View</span></a>
								'.$btn.'
							</th>
					</tr>';
				}?>
      ...
      <tfoot>
          ...
          <th> <?=number_format($payment['rm_pay'],2)?></th>
           <th></th>
        </tr>
      </tfoot>
    </table>
...
<div class="row-fluid">
  <div class="span6">
    <div class="page-header">Coin Wallet Withdrawal</div>
    <table class="table table-striped table-bordered">
      <thead>
      		<tr>
          		<th>Id</th>
  ...
  <div class="span6">
    ...
    <table class="table table-striped table-bordered">
      ...
      <tfoot>
      		<tr>
          		<th>Total</th>
          		<th><?=number_format($payment['rm_withdrawal'],2)?></th>
          		<th></th>
        	</tr>
      <tfoot>
    </table>
