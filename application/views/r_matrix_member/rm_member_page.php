...
<script>
	$(document).on('click','.request_for_kdk_code',function(e){
		 e.preventDefault()
...
<div class="row-fluid ">
  <div class="span12">
    <div class="pull-left">
      <?php if(!$rm_check['in_tree'] && !$rm_check['in_kdk']){?>
      <a class="show-pop-event" href="<?=base_url()?>index.php/<?=$this->uri->segment(1)?>/request_kdk_popup/" class="btn-right"><span class="label label-important">Request For KDK Code</span></a> <br />
      <span class="show_msg_request"></span>
      <?php } ?>
    </div>
    <?php if(!$rm_check['in_tree']){?>
    	<div class="pull-right">
      <form action="<?=base_url()?>index.php/<?=$this->uri->segment(1)?>/view" method="post" id="frmsubmit2" style="margin:0px;">
        <label style="margin-top:9px;float:left;margin-right:5px;"><strong>Enter KDK Code</strong></label>
        <input type="text" name="kdk_code" id="kdk_code" value="<?=$rm_check['code']?>" />
  ...
  <div class="span12">
    <div class="primary-head">
      <h3 class="page-header">
        ...
        <a href="<?=base_url()?>index.php/<?=$this->uri->segment(1)?>/edit_page/<?=$contain[0]['secret_page_code']?>" class="btn-right"><span class="label label-success">Edit Page</span></a>
        <?php } ?>
      </h3>
    </div>
  </div>
