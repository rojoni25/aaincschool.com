<?php
	if(count($upling_chain)>=2){
		$next_top=$upling_chain[count($upling_chain)-2]['usercode'];
	}else{
		$next_top=1;
	}	
?>
...
<script>
	$(document).on('click', '.open_popup', function (e) {
			var url='<?php echo vma_ad();?>tree_pop/tree_popup/<?=$next_top?>';
			e.preventDefault();
...
<div class="row-fluid">
  <div class="span12">
    <form action="<?=vma_ad()?><?=$this->uri->rsegment(1)?>/approve_insert" method="post">
