...
class cornjob_five_by_three extends CI_Controller {
	...
	public function index()
	{
		...
		$data = array();	
		$payment_check=$this->cornjob_daily_model->check_payment_date('5by3','paid');
		
		if(count($payment_check)>0){
			echo 'Today '.date('d-m-Y').' daily payment 5 x 3 is done';
			exit;
		}	
		
		
		$all_payment 		= 	$this->cornjob_daily_model->get_all_payment('5by3');
		...
		{			
				...
				$one_level=$this->cornjob_daily_model->tree_upling($all_payment[$i]['usercode'],'uplingmember5_3');
				$two_level=$this->cornjob_daily_model->tree_upling($one_level,'uplingmember5_3');
				$three_level=$this->cornjob_daily_model->tree_upling($two_level,'uplingmember5_3');
				
				
				$this->auto_load_model->master_balance_update_paid('5by3daily','main_balance',$one_level,$pay_level_one[0]['setting_value']);
				...
				//****Level Two Payment****//
				$this->auto_load_model->master_balance_update_paid('5by3daily','main_balance',$two_level,$pay_level_two[0]['setting_value']);
				$pay['paymentcode']	=	$all_payment[$i]['usercode'];
				...
				//****Level Three Payment****//
				$this->auto_load_model->master_balance_update_paid('5by3daily','main_balance',$three_level,$pay_level_three[0]['setting_value']);
				$pay['paymentcode']	=	$all_payment[$i]['usercode'];
				...
				//****Virtual Balance Minus****//
				$this->auto_load_model->master_balance_update('5by3',$all_payment[$i]['usercode'],$total_payment,'minus');
				
				$withdrawal['usercode']			=	$all_payment[$i]['usercode'];
			  	$withdrawal['balance_type']		=	'5by3';
