...
class cornjob_five_by_three_free extends CI_Controller {
	...
	public function index()
	{
		...
		$payment_check=$this->cornjob_daily_model->check_payment_date('5by3','free');
		
		$all_payment 		= 	$this->cornjob_daily_model->get_all_payment_free('5by3');
		...
		for($i=0;$i<count($all_payment);$i++)
		{
				...
				if($result[0]['tree5_1']=='0'){	$usercode='1';}
				else{$usercode=$result[0]['tree5_1'];}
				$this->auto_load_model->master_balance_update_multi('5by3daily','5by3level1',$usercode,$pay_level_one[0]['setting_value']);
				...
				//****Level Two Payment****//
				if($result[0]['tree5_2']=='0'){	$usercode='1';}
				else{$usercode=$result[0]['tree5_2'];}
				$this->auto_load_model->master_balance_update_multi('5by3daily','5by3level2',$usercode,$pay_level_two[0]['setting_value']);
				$pay['paymentcode']	=	$all_payment[$i]['usercode'];
				$pay['usercode']	=	$usercode;
				$pay['level']		=	'2';
				$pay['amount']		=	$pay_level_one[0]['setting_value'];
			    ...
			    //****Level Three Payment****//
				if($result[0]['tree5_3']=='0'){	$usercode='1';}
				else{$usercode=$result[0]['tree5_3'];}
				$this->auto_load_model->master_balance_update_multi('5by3daily','5by3level3',$usercode,$pay_level_three[0]['setting_value']);
				$pay['paymentcode']	=	$all_payment[$i]['usercode'];
				$pay['usercode']	=	$usercode;
				$pay['level']		=	'3';
				$pay['amount']		=	$pay_level_one[0]['setting_value'];
				...
				$this->auto_load_model->master_balance_update_free('5by3',$all_payment[$i]['usercode'],$total_payment,'minus');
