...
class cornjob_reverse_five_by_three extends CI_Controller {
	...
	public function index()
	{
		...
		$all_payment 		= 	$this->cornjob_daily_model->get_all_reverse_payment('5_3');
		if(count($all_payment)>0){
			foreach ($all_payment as $pay) {
				$usercode = $pay['usercode'];
				$payment_check=$this->cornjob_daily_model->check_paid_reverse_payment_date('5by3','free',$usercode);
				if(count($payment_check)>0)
				{
					...
					$remainwallet = $this->cornjob_daily_model->get_paid_reverse_remain_wallet($usercode,'5_3');
					...
					$uplinememberlevelArr = $this->cornjob_daily_model->get_reverse_upling_chain($usercode,'uplingmember5_3');
					//level 1
