...
class cornjob_reverse_ten_by_three_free extends CI_Controller {
	...
	public function index()
	{
		...
		$all_payment 		= 	$this->cornjob_daily_model->get_all_reverse_payment_free('10_3');
		if(count($all_payment)>0){
			foreach ($all_payment as $pay) {
				$usercode = $pay['usercode'];
				$payment_check=$this->cornjob_daily_model->check_reverse_payment_date('10by3','free',$usercode);
				...
				//}else{
					...
					$remainwallet = $this->cornjob_daily_model->get_reverse_remain_wallet($usercode,'10_3');
					...
					$uplinememberlevelArr = $this->cornjob_daily_model->get_reverse_upling_chain_free($usercode,'uplingmember10_3');
					//level 1
					...
					$this->cornjob_daily_model->addItem($L1data,'tbl_free_bonus_wallet');
					//level 2
					...
					$this->cornjob_daily_model->addItem($L2data,'tbl_free_bonus_wallet');
					...
					$this->cornjob_daily_model->addItem($L3data,'tbl_free_bonus_wallet');
