...
class media_upload extends CI_Controller {
	...
	{
		$p=array('image','mp4','youtube','ppt','audio');
		...
		$data['result']	=	$this->media_upload_model->get_media_by_type($data['mediatype']);
	...
	{
			$files = $_FILES;
			if (!is_dir('../upload/media/file/')) {
				mkdir('../upload/media/file/', 0777, TRUE);
			}
			if (!is_dir('../upload/media/thum/')) {
				mkdir('../upload/media/thum/', 0777, TRUE);
			}
			$data=array();
			...
			if($this->input->post('type')=='image')
			{
				...
				$path=base_url();
				$config = array();
    			$config['upload_path'] = '../upload/media/file';
				...
				if($files['post_img']['name']!='')
				{
					...
					$data['media_link']	=	$path.'upload/media/file/'.$fileName;
					$data['imgthum']	=	$path.'upload/media/thum/'.$fileName;
					$this->media_upload_model->addItem($data,'media_gallery');
				}
				...
				if($filetype=='image')
				{
						...
						$image_config['new_image'] = '../upload/media/thum/'.$fileName;
			...
			header('Location: '.base_url().'index.php/media_upload?type='.$this->input->post('type'));
			exit;
	}
	...
	{
		$data['status']='Delete';
		$this->media_upload_model->update($data,'media_gallery','gallerycode',$this->uri->segment(3));
