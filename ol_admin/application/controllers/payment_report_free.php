...
class payment_report_free extends CI_Controller {
	...
	public function due_payment_report()
	{
		...
		$this->load->view('report_due_payment_free');
	...
	function free_payment_daily()
	{
		...
		$this->load->view('comman/topheader',$data);
		$this->load->view('comman/header');
		$this->load->view('free_payment_daily_view');
		$this->load->view('comman/footer');
	}
	...
	function listing_paid_pay(){
		...
		$html='';
		$result=$this->ObjM->get_listing_free_pay($eid);
		for($i=0;$i<count($result);$i++)
		{
			$arr=array(
			...
			$total_amount=$this->ObjM->free_member_total_amount_get($arr);
			
			$pay='<a class="payment_form" href="'.base_url().'index.php/payment_report_free/get_free_from/'.$result[$i]['usercode'].'/'.$eid.'/'.$_POST['report_type'].'">'.$total_amount[0]['total'].'</a>';
			$html.='<tr>
						<td>'.$result[$i]['usercode'].'</td>
						<td>'.$result[$i]['fname'].' '.$result[$i]['lname'].'</td>
						<td>'.$result[$i]['username'].'</td>
	...
	function get_free_from($usercode, $time, $report_type){
		$arr=array(
		 	'usercode'	=>	$usercode,
			'timestm'	=>	$time,
			'pay_type'  => $report_type,
		);
		$result=$this->ObjM->get_free_from($arr);
