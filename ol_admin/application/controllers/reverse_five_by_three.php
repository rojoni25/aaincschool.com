...
class reverse_five_by_three extends CI_Controller {
	...
	function get_tree($code=''){
		$result=$this->tree_view_model->get_node_reverse_five_by_three_by_id($code);
		$topnode=$this->tree_view_model->userdt_by_code($code);
		
		$tree_arr=array();
		$url=''.base_url().'index.php/reverse_five_by_three/tree/';
		...
		if(isset($result[0])){
			...
			$room[0]=$this->tree_view_model->get_node_reverse_five_by_three_by_id($result[0]['usercode']);
		}
		else{
		...
		if(isset($result[1])){
			...
			$room[1]=$this->tree_view_model->get_node_reverse_five_by_three_by_id($result[1]['usercode']);
		}
		else{
		...
		if(isset($result[2])){
			...
			$room[2]=$this->tree_view_model->get_node_reverse_five_by_three_by_id($result[2]['usercode']);
		}
		else{
		...
		if(isset($result[3])){
			...
			$room[3]=$this->tree_view_model->get_node_reverse_five_by_three_by_id($result[3]['usercode']);
		}
		else{
		...
		if(isset($result[4])){
			...
			$room[4]=$this->tree_view_model->get_node_reverse_five_by_three_by_id($result[4]['usercode']);
		}
		else{
	...
	function drow_breadcrumb_level($code){
		...
		while(1){
			$result=$this->tree_view_model->get_reverse_breadcrumb_level($code);
			
			$user['bread'][]=$result;
			if($result[0]['uplingmember3_3']==$code){
				$result=$this->tree_view_model->get_reverse_breadcrumb_level($result[0]['uplingmember3_3']);
				$user['bread'][]=$result;
				break;	
			}
			if(!isset($result[0])){
