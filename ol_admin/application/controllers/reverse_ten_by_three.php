...
class reverse_ten_by_three extends CI_Controller {
	...
	function get_tree($code=''){
		$result=$this->tree_view_model->get_node_reverse_ten_by_three_by_id($code);
		$topnode=$this->tree_view_model->userdt_by_code($code);
		
		$tree_arr=array();
		$url=''.base_url().'index.php/reverse_ten_by_three/tree/';
	...
	function drow_dropdowntree($code){
		...
		$arr_old=$this->tree_view_model->get_node_ten_by_three_by_id($code);
		$arr=array();
	...
	function drow_breadcrumb_level($code){
		...
		while(1){
			...
			if($result[0]['uplingmember10_3']==$this->session->userdata['logged_ol_member']['usercode']){
				$result=$this->tree_view_model->get_reverse_breadcrumb_level($result[0]['uplingmember10_3']);
				$user['bread'][]=$result;
				break;	
			}
			if(!isset($result[0])){
