...
class reverse_three_by_three extends CI_Controller {
	...
	function get_tree($code=''){
		$result=$this->tree_view_model->get_node_reverse_three_by_three_by_id($code);
		//var_dump($result);
		//exit;
		$topnode=$this->tree_view_model->userdt_by_code($code);
		
		$tree_arr=array();
		$url=''.base_url().'index.php/reverse_three_by_three_free/tree/';
		...
		if(isset($result[0])){
			...
			$room[0]=$this->tree_view_model->get_node_reverse_three_by_three_by_id($result[0]['usercode']);
		}
		else{
		...
		if(isset($result[1])){
			...
			$room[1]=$this->tree_view_model->get_node_reverse_three_by_three_by_id($result[1]['usercode']);
		}
		else{
		...
		if(isset($result[2])){
			...
			$room[2]=$this->tree_view_model->get_node_reverse_three_by_three_by_id($result[2]['usercode']);
		}
		else{
		...
		for($i=0;$i<count($lev3);$i++){
			$rg=$this->tree_view_model->get_node_reverse_three_by_three_by_id($lev3[$i]['code']);
			if(isset($rg[0])){
	...
	function drow_dropdowntree($code){
		...
		$arr_old=$this->tree_view_model->get_node_three_by_three_by_id($code);
		$arr=array();
