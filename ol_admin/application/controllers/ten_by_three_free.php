...
class ten_by_three_free extends CI_Controller {
	...
	function get_tree($code=''){
		$result=$this->tree_view_free_model->get_node_ten_by_three_by_id($code);
		$topnode=$this->tree_view_free_model->userdt_by_code($code);
		
		$tree_arr=array();
		$url=''.base_url().'index.php/ten_by_three_free/tree/';
	...
	function drow_breadcrumb_level($code){
		...
		while(1){
			...
			if($result[0]['uplingmember10_3']==$this->session->userdata['logged_ol_member']['usercode']){
				$result=$this->tree_view_free_model->get_breadcrumb_level($result[0]['uplingmember10_3']);
				$user['bread'][]=$result;
				break;	
			}
			if(!isset($result[0])){
