...
class r_matrix extends CI_Controller {
		...
		for($i=0;$i<count($member_list);$i++){
				...
				// For Angel Position Member
					if($member_list[$i]['usercode'] == MATRIX_SYSTEM_ADMIN){
						...
						$totalposition=$this->ObjM->get_multi_position($curusercode);
	...
	function test(){
		$idcode = 77;
		$curposition=$this->ObjM->get_multi_position($idcode);
		echo count($curposition);
		echo '<pre>';
		print_r($curposition);
		echo '</pre>';
		die();
		$top_third_member	=	$this->get_top_third_member($idcode);
		print_r($top_third_member);
		$this->get_total_downline($top_third_member['idcode']);
		echo $this->tot_downline;
		$top_system=array('usercode'=>MATRIX_SYSTEM_ADMIN);
