...
class r_matrix_tree_free extends CI_Controller {
	...
	function __construct()
 	{
		...
		$this->load->model('matrix_comman/r_matrix_tree_model_free','ObjM',TRUE);
	...
	public function view($eid)
	{
		...
		$this->load->view(''.MATRIX_FOLDER.'/admin/r_matrix_tree_free_view',$data);
	...
	protected function tree_elements($arr_mem){
		...
		for($i=0;$i<count($arr_mem);$i++){
			...
			if(isset($result[0])){
				...
				if($result[0]['angel_position']>0){
					$result[0]['img']="<img src='".base_url()."/asset/images/multitree/orange.png'  />";
					$angel_mem = $this->ObjM->search_member_from_angel($result[0]['angel_position']);
					$posArr = $this->ObjM->get_current_position($result[0]['idcode'],$result[0]['usercode']);
					$posno = $posArr['cntno'];
					$result[0]['name'] = 'Angel vs '.$angel_mem['fname'].' '.$posno;
				}else{
					$result[0]['img']="<img src='".base_url()."/asset/images/multitree/admin.png'  />";
					if($result[0]['usercode']!=MATRIX_SYSTEM_ADMIN){
						$posArr = $this->ObjM->get_current_position($result[0]['idcode'],$result[0]['usercode']);
						$posno = $posArr['cntno']>1?$posArr['cntno']:'';
						$result[0]['name'] = $result[0]['name'].' '.$posno;
					}
				}
			...
			if(isset($result[1])){
				...
				if($result[1]['angel_position']>0){
					$result[1]['img']="<img src='".base_url()."/asset/images/multitree/orange.png'  />";
					$angel_mem = $this->ObjM->search_member_from_angel($result[1]['angel_position']);
					$posArr = $this->ObjM->get_current_position($result[1]['idcode'],$result[0]['usercode']);
					$posno = $posArr['cntno'];
					$result[1]['name'] = 'Angel vs '.$angel_mem['fname'].' '.$posno;
				}else{
					$result[1]['img']="<img src='".base_url()."/asset/images/multitree/admin.png'  />";
					if($result[1]['usercode']!=MATRIX_SYSTEM_ADMIN){
						$posArr = $this->ObjM->get_current_position($result[1]['idcode'],$result[1]['usercode']);
						$posno = $posArr['cntno']>1?$posArr['cntno']:'';
						$result[1]['name'] = $result[1]['name'].' '.$posno;
					}
				}
