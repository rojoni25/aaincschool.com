	...
	function listing($id=''){
		...
		for($i=0;$i<count($result);$i++)
		{
			$ref		=$this->upgrade_request_model->get_member_by_code($result[$i]['referralid_free']); 
		 	$refrequest = $this->upgrade_request_model->check_request_send($result[$i]['referralid_free']);
			$refcolor = '';
			if($refrequest[0]['payment']=='Y')
			{
				$refcolor = 'green';
			}else{
				$refcolor = 'red';
			...
			if($result[$i]['payment']=='Y')
			{
				if($result[$i]['confirm']=='Y'){
					...
					$payment='Paid';
					$cls='trconfirm';
					$pay_btn='<a href="'.base_url().'index.php/upgrade_request/confirm_payment/'.$result[$i]['usercode'].'" class="confirm-pay"><button class="sendbtn btn-success">Confirm Payment</button></a>';
					$pay_date=date("d-m-Y", strtotime($result[$i]['payment_dt']));
				}
			}
			else{
			...
			$html .='<tr class="'.$cls.'">
						...
						<td style="color:'.$refcolor.'">'.$ref[0]['fname'].' '.$ref[0]['lname'].'</td>
