...
class tree_pop extends CI_Controller {
	...
	public function tree_popup($eid)
	{
		...
		$this->load->view(VMA_FOLDER.'tree_popup',$data);
	...
	function drow_tree($eid)
	{
		...
		if(isset($node[0])){
			$html.='<div class="tree-node pull-left">
				...
				<p class="center"><a class="tree-user" href="'.vma_base().$this->uri->rsegment(1).'/get_tree/'.$node[0]['usercode'].'">'.$node[0]['name'].'</a></p>
			</div>';
		}else{
			$html.=$empty;
		}
		...
		<p class="center"><a class="tree-user" href="'.vma_base().$this->uri->rsegment(1).'/get_tree/'.$node[1]['usercode'].'">'.$node[1]['name'].'</a></p>
		</div>';
		}else{
			$html.=$empty;
		}
		...
		<p class="center"><a class="tree-user" href="'.vma_base().$this->uri->rsegment(1).'/get_tree/'.$node[2]['usercode'].'">'.$node[2]['name'].'</a></p>
		</div>';	
		}else{
			$html.=$empty;
		}
		...
		$result		=	$this->vma_class->upling_chain($eid);
			...
			for($i=0;$i<count($result);$i++){
				if($i==count($result)-1){$html.=$result[$i]['name'];}
					...
					$html.='<a class="tree-user" href="'.vma_base().$this->uri->rsegment(1).'/get_tree/'.$result[$i]['usercode'].'">'.$result[$i]['name'].'</a>&nbsp;&nbsp;<i class="icon-angle-right"></i>&nbsp;&nbsp;';
				}		
			}
		return $html;
	}
