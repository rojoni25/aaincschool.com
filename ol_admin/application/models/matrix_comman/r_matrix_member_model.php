...
Class r_matrix_member_model extends CI_Model
{
	...
	function getAll_active()
 	{	
		...
		$aColumns = array(''.MATRIX_TABLE_PRE.'matrix.idcode',''.MATRIX_TABLE_PRE.'matrix.usercode','user1.fname', 'user1.lname', 'user1.emailid','user1.mobileno');
		...
		////////////
			//if ( $sWhere == "" ){$sWhere = "WHERE '.MATRIX_TABLE_PRE.'matrix.usercode!='-1'";}
			//else{$sWhere .= " AND '.MATRIX_TABLE_PRE.'matrix.usercode!='-1'";	}
			...
			CONCAT(user2.fname, " ", SUBSTRING(user2.lname,1,1)) as name2, user2.username as ref_username, user2.usercode as ref_code, '.MATRIX_TABLE_PRE.'matrix.* 
			FROM ('.MATRIX_TABLE_PRE.'matrix) 
			LEFT JOIN membermaster user1 ON user1.usercode = '.MATRIX_TABLE_PRE.'matrix.usercode 
			LEFT JOIN membermaster user2 ON user2.usercode = '.MATRIX_TABLE_PRE.'matrix.upling_member
			'.$sWhere.'
	...
	function get_multi_position_detail($eid)
	{
		...
		$this -> db -> join(''.MATRIX_TABLE_PRE.'member_payment pay_rm','pay_rm.position = '.MATRIX_TABLE_PRE.'matrix.idcode and pay_rm.wallet_type="RM"','left');
	...
	function cycle_report(){
		$this -> db -> select(''.MATRIX_TABLE_PRE.'member_payment.*');
		$this -> db -> select('CONCAT(membermaster.fname, " ", SUBSTRING(membermaster.lname,1,1)) as name, membermaster.emailid, membermaster.phone_no, membermaster.status',FALSE);
		$this -> db -> join('membermaster','membermaster.usercode = '.MATRIX_TABLE_PRE.'member_payment.usercode','left');
   		$this -> db -> from(''.MATRIX_TABLE_PRE.'member_payment');
   		$this -> db -> where(''.MATRIX_TABLE_PRE.'member_payment.wallet_type','COIN');
		$this -> db -> order_by(''.MATRIX_TABLE_PRE.'member_payment.id', 'asc');
		$query = $this -> db -> get();
    	$the_content = $query->result_array();
    	return $the_content;
	}	
