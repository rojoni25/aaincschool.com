<?php
Class virtual_balance_model extends CI_Model
{
 	...
 	{	
		...
		////////////
			if ( $sWhere == "" ){$sWhere = "WHERE master_balance_sheet.".$eid." >= '".$balance."' AND membermaster.status = 'Active' AND membermaster.usercode!='1'";}
			else{$sWhere .= " AND master_balance_sheet.".$eid." >= '".$balance."' AND membermaster.status = 'Active' AND membermaster.usercode!='1'";	}
			
		$sQuery = "
				SELECT membermaster.*, master_balance_sheet.".$eid." as balance FROM (membermaster) 
				LEFT JOIN master_balance_sheet ON master_balance_sheet.usercode = membermaster.usercode 
				$sWhere
				$sOrder
				$sLimit";
