 <script>
    	$(document).on('change','#month_name',function(e){
			...
			var url='<?=vma_base()?><?=$this->uri->rsegment(1)?>/get_income_report/<?=$eid?>/'+date;
			$.ajax({url:url,
				beforeSend: function(){
   				},
   				complete: function(){
		...
		$(document).on('click','.date_detail_view',function(e){
			...
			var url	=	'<?=vma_base()?><?=$this->uri->rsegment(1)?>/income_report_datewise/<?=$eid?>/'+val;
    ...
    <div class="primary-head">
      <h3 class="page-header">VMA Member Main Wallet
      			<div class="pull-right">
                    ...
                    <a href="<?=vma_base()?>member/detail/<?=$result['usercode']?>"><button class="btn btn-round-min btn-warning" type="button"><span><i class="icon-eye-open"></i></span></button></a>
                </div>
      </h3>
    </div>
    <ul class="breadcrumb">
