    ...
    <div class="primary-head">
      <h3 class="page-header">VMA Member
      	<div class="pull-right">
        	<a href="<?=vma_base()?>tree/view/<?=$member['usercode']?>"><button class="btn btn-round-min btn-danger"><span><i class="icon-group"></i></span></button></a>
        </div>
      </h3>
    </div>
    ...
    <ul class="breadcrumb">
	  ...
	  for($i=0;$i<count($upling_chain);$i++){
			...
			if($i<count($upling_chain)-1){
				echo '<li><a href="'.vma_base().''.$this->uri->rsegment(1).'/detail/'.$upling_chain[$i]['usercode'].'">'.$upling_chain[$i]['name'].'</a> <i class="icon-angle-right"></i>';		
			}
			else{
    ...
    <table class="table">
      ...
      <?php
			...
			if($this->vma_class->manully_qulified($member['usercode'])){
				$manully = 'Yes';
				$manully.= '&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.vma_base().$this->uri->rsegment(1).'/unqulified/'.$member['usercode'].'">Unqulified</a>';
			}else{
				$manully='No';
				$manully.= '&nbsp;&nbsp;&nbsp;&nbsp;<a href="'.vma_base().$this->uri->rsegment(1).'/qulified/'.$member['usercode'].'">Qulified</a>';
			}
...
<div class="row-fluid">
	...
	<table class="table">
          ...
          <tr>
            ...
            <td width="1%">:</td>
            <td width="75%"><a href="<?=vma_base()?>report/virtual_wallet/<?=$member['usercode']?>">$<?=$member_balance[0]['virtual_in']?></a></td>
          </tr>
          
          <tr>
            <td>Virtual Payment</td>
            <td>:</td>
            <td><a href="<?=vma_base()?>report/virtual_wallet_user_detail/<?=$member['usercode']?>">$<?=$member_balance[0]['virtual_out']?></a></td>
          </tr>
          ...
          <tr>
            <td></td>
            <td></td>
           <td><a href="<?=vma_base()?>report/virtual_wallet/<?=$member['usercode']?>">Virtual Account</a></td>
          </tr>
	...
	<table class="table">
          ...
          <tr>
            <td></td>
            <td></td>
           <td><a href="<?=vma_base()?>report/main_wallet/<?=$member['usercode']?>">Main Account</a></td>
          </tr>
