          ...
          <tbody>
            	...
            	<?php for($i=0;$i<count($result);$i++) { 
						...
						if(isset($result[$i]['payment_code']) && $result[$i]['payment_code']!=''){
							$btn='<a href="'.vma_base().$this->uri->rsegment(1).'/approve/'.$result[$i]['request_code'].'"><span class="label label-success">Join</span></a>';		
						}else{
							
							if($this->comman_fun->check_record('vma_message',array('type'=>'payment_confirm','status'=>'Active','usercode'=>$result[$i]['usercode']))){
								$btn='<a href="'.vma_base().$this->uri->rsegment(1).'/payment_confirm/'.$result[$i]['request_code'].'"><span class="label label-important">Payment Confirm</span></a>&nbsp;&nbsp;';	
								$btn.='<a href="'.vma_base().'general/payment_detail/'.$result[$i]['usercode'].'"><span class="label label-success">View Payment</span></a>';
							}else{
								$btn='Not Send';	
							}
